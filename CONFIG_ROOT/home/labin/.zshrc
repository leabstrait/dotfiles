...
function dotfiles() {
    ...
    if [ "$1" = "track" ] || [ "$1" = "untrack" ]; then
        ...
        if [ $command = "track" ]; then
            if [ -f $file ]; then
                # copy the file to the dotfiles directory preserving its path
                echo "cp $file $HOME/dotfiles/CONFIG_ROOT$file"
                cp $file $HOME/dotfiles/CONFIG_ROOT$file

                # also make a backup
                echo "mv $file $HOME/dotfiles/CONFIG_ROOT$file.bak"
                mv $file $HOME/dotfiles/CONFIG_ROOT$file.bak

                # create a symlink to the dotfiles directory
                echo "ln -s $HOME/dotfiles/CONFIG_ROOT$file $file"
                ln -s $HOME/dotfiles/CONFIG_ROOT$file $file
            else
        ...
        if [ $command = "untrack" ]; then
            if [ -L ${file#"$HOME/dotfiles/CONFIG_ROOT"} ]; then
                # move the file from the dotfiles directory into the original
                # directory preserving its path and name (if it exists)
                echo "mv $file  ${file#"$HOME/dotfiles/CONFIG_ROOT"}"
                mv $file  ${file#"$HOME/dotfiles/CONFIG_ROOT"}
        ...
        if [ -d "$HOME/dotfiles/.git"  ]; then
            git --git-dir=/home/labin/dotfiles/.git --work-tree=/home/labin/dotfiles $@
        fi
    fi
}
