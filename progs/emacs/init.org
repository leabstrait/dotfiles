...
(use-package emacs
  ...
  ;; (dired-kill-when-opening-new-dired-buffer t) ; Dired doesn't create new Dired buffer
...
(defun start/tangle-config ()
  ...
  (let ((init-org
    ...
    (unless (file-exists-p init-org)
      (user-error "Cannot find %s" init-org))

    (org-babel-tangle-file init-org init-el "emacs-lisp")

    (message "Successfully tangled %s -> %s"
             init-org
             init-el)))

(defun start/open-init-file ()
  "Open the init.org configuration file."
  (interactive)
  (find-file
   (expand-file-name "init.org" user-emacs-directory)))

(defun start/reload-config ()
  "Tangle init.org and reload the generated init.el."
  (interactive)
  ...
  (let ((init-el
    ...
    (message "Configuration reloaded from init.org")))
#+END_SRC
...
(custom-set-faces
 ...
 `(mode-line
   ((t (:background ,my/panel-bg
        ...
        :box (:line-width 1 :color ,my/accent-primary)))))
 `(mode-line-inactive
   ((t (:background ,my/dark-bg
        :foreground "#8f9ba8"
        :box (:line-width 1 :color "#2d3748")))))
 `(mode-line-buffer-id
   ((t (:foreground ,my/accent-primary :weight bold))))
...
(with-eval-after-load 'helm
  (custom-set-faces
   ...
   `(helm-candidate-number
     ((t (:foreground ,my/accent-muted :weight bold))))
   `(helm-separator
...
(with-eval-after-load 'markdown-mode
  (custom-set-faces
   ...
   `(markdown-header-face-2
     ((t (:foreground "#38bdf8" :weight bold))))))
#+END_SRC
...
NOTE: This config primarily relies on Helm (Section above).
...
Future configuration space for Docker / Kubernetes / Ansible beyond
the currently enabled Dockerfile and YAML support.
...
Future language-specific configuration can be added here without
disturbing the core configuration structure.
